...
*				0表示失败
**/
int Http::httpGet(const char* strUrl, char* strResponse)
{
...
*				0表示失败
**/
int Http::httpPost(const char* strUrl, const char* strData, char* strResponse)
{
...
//执行HTTP请求，GET或POST
int Http::httpRequestExec(const char* strMethod, const char* strUrl, const char* strData, char* strResponse)
{
	//判断URL是否有效
...
//构建HTTP消息头
char* Http::httpHeadCreate(const char* strMethod, const char* strUrl, const char* strData)
{
...
//发送HTTP请求并接受响应
char* Http::httpDataTransmit(char *strHttpHead, const int iSockFd)
{
...
char* Http::getHostAddrFromUrl(const char* strUrl)
{	
	char url[URLSIZE] = {0};
	strcpy(url, strUrl);
...
char* Http::getParamFromUrl(const char* strUrl)
{	
	char url[URLSIZE] = {0};
	strcpy(url, strUrl);
...
//从HTTP请求URL中获取端口号
int Http::getPortFromUrl(const char* strUrl)
{
	int iPort = -1;
...
//从HTTP请求URL中获取IP地址
char* Http::getIPFromUrl(const char* strUrl)
{
	char* strHostAddr = getHostAddrFromUrl(strUrl);
...
//检查SocketFd是否为可写不可读状态
int Http::socketFdCheck(const int iSockFd)
{
...
//打印输出
void Http::debugOut(const char *fmt, ...)
{
#ifdef __DEBUG__
	va_list ap;
	va_start(ap, fmt);
...
int Http::m_iSocketFd = INVALID_SOCKET;
