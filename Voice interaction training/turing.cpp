...
*				0表示失败
**/
int Turing::HttpGet(const char* strUrl, char* strResponse)
{
...
*				0表示失败
**/
int Turing::HttpPost(const char* strUrl, const char* strData, char* strResponse)
{
...
//执行HTTP请求，GET或POST
int Turing::HttpRequestExec(const char* strMethod, const char* strUrl, const char* strData, char* strResponse)
{
	//判断URL是否有效
...
//构建HTTP消息头
char* Turing::HttpHeadCreate(const char* strMethod, const char* strUrl, const char* strData)
{
...
//发送HTTP请求并接受响应
char* Turing::HttpDataTransmit(char *strHttpHead, const int iSockFd)
{
...
char* Turing::GetHostAddrFromUrl(const char* strUrl)
{	
	char url[URLSIZE] = {0};
	strcpy(url, strUrl);
	...
	memset(strHostAddr, 0, iLen+1);//初始化新申请的内存
...
char* Turing::GetParamFromUrl(const char* strUrl)
{	
	char url[URLSIZE] = {0};
	strcpy(url, strUrl);
...
//从HTTP请求URL中获取端口号
int Turing::GetPortFromUrl(const char* strUrl)
{
	int iPort = -1;
...
//从HTTP请求URL中获取IP地址
char* Turing::GetIPFromUrl(const char* strUrl)
{
	char* strHostAddr = GetHostAddrFromUrl(strUrl);
...
//检查SocketFd是否为可写不可读状态
int Turing::SocketFdCheck(const int iSockFd)
{
...
//打印输出
void Turing::DebugOut(const char *fmt, ...)
{
#ifdef __DEBUG__
	va_list ap;
	va_start(ap, fmt);
...
int Turing::m_iSocketFd = INVALID_SOCKET;
